...
\begin{enumerate}
  \item % TODO(review-2026-05-21): before submission, run a Google Scholar + arXiv search on "metadatafication" + "version control" / "infrastructure transparency" + "AI agents" to confirm no prior use of the exact term. Star (1996), Edwards (2003), and Bowker (1999) are already cited as the conceptual base.
We define and characterize \textbf{metadatafication} as a general pattern of infrastructure evolution, grounding it in historical precedents from networking, storage, and media technologies.
...
% TODO(review-2026-05-21): consider operationalizing Phase 1→2 and Phase 2→3 transition thresholds in measurable terms (e.g., share of agent-authored commits above which a project is classified as Phase 2). Currently the phases are qualitative; reviewers may push for concrete cutoffs.
% TODO(market-pulse-2026-05-29): compare the three-phase progression here against Microsoft Agent Framework's git-as-memory metaphor (COMMIT/BRANCH/MERGE primitives for long-horizon agent context, released Q2 2026). The vocabulary overlap is significant — clarify whether their framing treats git operations as a *memory primitive for agents* (orthogonal to our metadatafication direction) or as *infrastructure that itself becomes invisible* (the same direction). Source: search for "Microsoft Agent Framework git-like operations" + 2026.
...
% TODO(market-pulse-2026-05-29): add a short comparison paragraph against the local-machine *content-search* layer that emerged in 2026 Q2 — CASS (Coding Agent Session Search; ~800 stars), Agent Sessions (native macOS), and cli-continues all index transcripts from Claude/Cursor/Codex/Gemini for unified search and replay. Canary shares the on-disk data source (\texttt{\textasciitilde/.claude/projects/} et al.) but joins it with multi-repo git state rather than indexing the conversations themselves. Worth surfacing because reviewers might otherwise read those tools as direct comparators for the push-leakage instrument.
...
A second instrument added to Canary measures the gap between agent activity and remote propagation \textit{in Claude-Code-tracked operator workflows}. The scope qualifier is load-bearing: the instrument joins per-repo git working-tree state with session transcripts persisted under \texttt{\textasciitilde/.claude/projects/}, which is the on-disk record produced by the Claude Code CLI and Desktop. Equivalent transcript layers exist for Cursor (\texttt{\textasciitilde/.cursor/}), Codex CLI, and Gemini CLI, but their schemas differ enough that a single parser does not cover them; we leave the cross-tool extension to a follow-up study and confine the empirical claims in this section to the Claude-Code-tracked surface. The mechanism we describe (operator-attention budget failing to scale with portfolio breadth) is not specific to any one tool---only the measurement is.

We define three quantities: \textit{Agent-Push Latency} (APL, time from the last agent session that touched a repo to the present, while the repo remains ahead of upstream), \textit{Metadata-Invisibility Period} (MIP, how long the oldest unpushed commit has been sitting unpropagated), and a \textit{Push Leakage Rate} (PLR, the fraction of repos in MIP exceeding a threshold). MIP is the operational analogue of \citet{star1996steps}'s prediction that infrastructure becomes visible only upon breakdown: while the unpushed commits remain unsurfaced, git is functioning as transparent background data; when an external scan enumerates them, the breakdown becomes legible. We illustrate the instrument with one author's $N=57$ repository portfolio under a single root.
...
\paragraph{Eleven-day follow-up.}
To test whether the recovery state is stable or whether the portfolio drifts back into leakage, we re-ran the same scanner against the same portfolio root 11 days after the post-intervention zero-baseline (run date 2026-05-21; 60 repositories scanned, 59 agent-touched). Only 1 repository sat above the leakage threshold (MIP = 13d 10h, ahead = 1 commit) yielding $\textrm{PLR}_\text{agent}=1.7\%$ --- a slow re-accumulation rather than a return to the baseline bulk-cluster. The trajectory $40.4\% \to 0\% \to 1.7\%$ supports the recoverability claim above and locates the steady-state leakage rate, in this operator's portfolio, an order of magnitude below the bulk-event signature. A second observation from the same follow-up: the working-tree staleness metric (UCP) shows a separate accumulation pattern with $\max\textrm{UCP}=74\,\text{days}\,4\,\text{hours}$ across 6 dirty repositories, decoupled from push-leakage --- consistent with our framing of UCP as a noisier lower-layer signal of the same attention-budget breakdown. The cross-repo attribution shape also shifts: of the 59 agent-touched repositories in the follow-up window, 54 received cwd-direct sessions and only 5 ($\approx 8.5\%$) were touched exclusively from a parent directory, compared with $\approx 87\%$ in the baseline. The shift is consistent with operator behavioural change post-intervention, but the underlying tooling implication of \S6.2 is unchanged: a session-completion CI/CD trigger that ignores command content would still miss the cross-only minority, and that minority can be expected to grow whenever a similar bulk operation recurs.
...
\paragraph{Falsifying condition.} The metadatafication thesis (Section~3) predicts that, in repositories that have adopted agent-era artifacts, the share of commit attention devoted to those artifacts should grow over time as the project transitions from Phase~1 (Active Tool) through Phase~2 (Assisted Tool). The thesis is operationally falsified by the metrics in this section if, in adopting developer-led projects (those with at least one agent-era file present), the shorter-window CAM is not greater than the longer-window CAM (i.e., 30d-CAM $\leq$ 365d-CAM) in $\geq 80\%$ of cases across at least two consecutive quarters. The acceleration finding reported below is non-trivial precisely because it could have failed: a project that created an AGENTS.md in Q1 2026 and never touched it again would show 30d-CAM $<$ 365d-CAM, contradicting the prediction. A symmetric falsification holds for ACR: if developer-led and foundation-governed projects show no statistically detectable difference after Holm correction at the conclusion of two such windows, the governance-mediation claim in Section~3.1 is unsupported.
...
% TODO(review-2026-05-21): the "refinement-phase" vs "creation-phase" bifurcation currently uses Ratio≈0.1 as an informal cutoff. If a future revision wants to elevate this from descriptive to predictive, pick a pre-registered threshold and report repos above/below it explicitly.
...
The 26-repository reference frame is a deliberately small audited sample suitable for governance classification; the ecosystem-level baseline is several orders of magnitude larger. By Q2 2026 the AGENTS.md format had been adopted by more than 60{,}000 public repositories on GitHub and was registered as a project of the Agentic AI Foundation, indicating that the convention has crossed the threshold from individual-developer experimentation into a coordinated standard. Our 26-repo audit therefore reads the *governance-mediated* layer of an adoption process whose absolute size is set elsewhere; it is not a population estimate. The 11 of 21 traditional-OSS adoption rate (52\%) in our sample is consistent with the broader 60{,}000-repository figure under the assumption that the highest-activity repositories adopt earlier than the long tail.
...
The MCP 2026-07-28 release candidate, locked 2026-05-21, gives the agent-readability infrastructure a near-term standardisation surface: W3C Trace Context propagation is now documented in the protocol's \texttt{\_meta} envelope under \texttt{traceparent}, \texttt{tracestate}, and \texttt{baggage}, so a host that participates in distributed tracing (Claude Code, Cursor CLI, Codex CLI, Gemini CLI) can stitch tool calls through MCP servers and into a downstream OpenTelemetry-compatible backend without per-vendor instrumentation \citep{mcp2026rc}. The same RC introduces a stateless transport (no \texttt{initialize/initialized} handshake; routing by \texttt{Mcp-Method} / \texttt{Mcp-Name} headers) and an asynchronous \texttt{tasks/get/update/cancel} pattern. These primitives directly support the recommendations above: the trace keys make session-completion CI/CD triggers observable across tools; the asynchronous task handle is the protocol-level shape of ``intent-level CI/CD triggered by agent session completion'' that the canary instrument operationalises only locally today.
...
The Claude Code source-map leak of 31 March 2026 made these questions empirically concrete \citep{anthropic2026codeleak}. The leak exposed approximately 1{,}900 internal source files and over 512{,}000 lines of code through \texttt{.map} files in an npm package, including the agentic architecture, the tool system, and 44 unreleased feature flags. The leak window coincided with malicious versions of the \texttt{axios} npm package being published with a remote-access trojan---and \texttt{axios} is a transitive dependency of Claude Code itself. Industry reporting in the same window observed that Claude-Code-assisted commits leaked secrets (API keys, passwords, credentials) at a 3.2\% rate, more than double the 1.5\% baseline across all public {GitHub} commits \citep{ai2026secretleakage}. We do not interpret either statistic as a defect of any particular vendor; they are a per-operator-attention-budget story. The metadatafication thesis predicts exactly this: when commit, push, and dependency update all become metadata the operator no longer inspects, the visibility surface for the kind of mistake the leak captured (secret in a build artifact; trojan in a transitive dependency) shrinks proportionally. The §5.4 push-leakage instrument and the host-allow-list pattern of canary's own outbound fetches (Section~7) are concrete responses to the same governance pressure: instruments that surface the otherwise-invisible portion of the operator's attention queue.
...
\textbf{Internal validity.} The reference repository sample was selected purposively, not randomly. We chose well-known projects across 7 ecosystems to maximize diversity, but the selection is vulnerable to post-hoc bias: we may have unconsciously included projects known to have adopted agent-era files. The governance classification (developer-led vs.\ foundation-governed) is operationalized based on whether the project's primary governance authority is a non-profit foundation or it follows a formal proposal process (Section~5.4). While this criterion is more objectively verifiable than ad-hoc assignment, edge cases exist (e.g., corporate-backed projects with informal but gatekept review processes), and independent validation of the classification was not performed. To bound the sensitivity quantitatively, we automate a leave-one-out reclassification check: for each of the 21 traditional repositories, we flip its label and recompute both the CAM and ACR Mann-Whitney tests under Holm correction. The full table is written to \texttt{experiments/results/statistical-tests-results.json} under \texttt{sensitivity}; pivotal repositories (those whose single flip changes a 0.05 decision in either test) are reported as a count in the same record. The 2026-05-29 run of the sensitivity check reports 12 of 21 reclassifications would flip the 0.05 decision in at least one of the two tests in Holm-vs-Holm space (10 flips affect CAM, 10 affect ACR, 8 affect both) --- a substantial dependence that should be read alongside the headline $p_\text{Holm}=0.034$ result rather than separately from it. We emphasise Holm-vs-Holm comparison here because the earlier 2026-05-21 implementation contrasted the raw baseline $p_\text{raw}$ against the LOO Holm-adjusted $p_\text{Holm}$; that comparison conflated the LOO flip with the Holm correction itself. On this dataset both comparisons happen to coincide at 12 pivotal repositories because the raw and Holm-adjusted baselines fall on the same side of $\alpha=0.05$, but the corrected comparison is the principled one for future reruns where the two baselines straddle the threshold. A reader who disagrees with our classification of any specific project can therefore see exactly how much the conclusion depends on that one judgement. The temporal CAM analysis uses overlapping windows (30d $\subset$ 60d $\subset$ 90d $\subset$ 180d $\subset$ 365d), so the observed ``acceleration'' could reflect a small number of recent file-creation events amplified in shorter windows rather than a sustained trend. Both Mann-Whitney U tests (CAM and ACR) reach significance at $p=0.017$, but with $n=21$ the tests have limited power, and the result is sensitive to governance classification choices (reclassifying two repositories shifted the ACR result from $p=0.060$ to $p=0.017$).
...
\textbf{Reliability.} All experiments query the GitHub API at a specific point in time (April 2026). Results are inherently non-reproducible in the strict sense: re-running the scripts will produce different numbers as new commits accumulate and repositories evolve. We mitigate this by archiving raw JSON results alongside the scripts in a replication package, to be deposited at Zenodo after the first revision pass; the current manuscript carries the placeholder DOI \href{https://doi.org/10.5281/zenodo.XXXXXXX}{10.5281/zenodo.XXXXXXX} which will be replaced with the live deposit identifier prior to camera-ready. Within the replication scripts themselves, the bootstrap resampler is now seeded (\texttt{mulberry32}, default seed 20260521, overridable via \texttt{STAT\_SEED}), so confidence intervals are reproducible at the third decimal given the same input snapshots. The bootstrap CIs provide a measure of sampling uncertainty but do not account for temporal instability of the underlying GitHub data.
% TODO(pre-camera-ready): replace 10.5281/zenodo.XXXXXXX above and in CITATION.cff with the real Zenodo deposit DOI; verify the deposit landing page renders the archived experiments/results/ snapshot.

\textbf{Single-developer vignette and researcher-as-subject.} The §5.4 push-leakage vignette is drawn from one developer's portfolio (the author's), and the operator who introduced the unpushed metadata, ran the scanner that surfaced it, and authored the analysis are the same person. Three concrete biases follow. (i) The portfolio is small ($N=57$) and the bulk-event signature observed (21 of 23 leaking repos clustered within a $\sim$3-hour window) reflects a single restructure operation; the cluster shape is informative about the mechanism but not about base rates across developers. (ii) Threshold choices (the 7-day MIP cutoff for the leakage classifier; the 30-day window for the recent-issues digest; the $\sim$3-hour spread used to characterise ``cluster'') are presented descriptively but not subjected to formal sensitivity analysis. The qualitative finding (degenerate MIP cluster + two outliers) is robust to threshold variation in the ranges we explored, but a future multi-developer study would need pre-registered thresholds. (iii) Identifying which observations count as ``breakdown moments'' versus ``routine catch-up'' is interpretive, and the author has thesis-confirmation incentive. We mitigate (iii) by reporting inverse cases (three agent-touched repositories not in the leakage set, including this paper's own repository), and by citing external corroboration: Claude Code project issues \href{https://github.com/anthropics/claude-code/issues/26725}{\#26725} and \href{https://github.com/anthropics/claude-code/issues/43730}{\#43730} document the worktree-leakage pattern independently of this study, and GitHub CLI \href{https://github.com/cli/cli/issues/12980}{\#12980} documents the command-cleanup-leakage pattern in the maintainers' own bug tracker.
