...
\textbf{Reliability.} All experiments query the GitHub API at a specific point in time (April 2026). Results are inherently non-reproducible in the strict sense: re-running the scripts will produce different numbers as new commits accumulate and repositories evolve. We mitigate this by archiving raw JSON results alongside the scripts in a replication package, deposited at Zenodo (\href{https://doi.org/10.5281/zenodo.XXXXXXX}{10.5281/zenodo.XXXXXXX}) so that any future reader has the exact dataset and source from which the vignette numbers were computed. The bootstrap confidence intervals provide a measure of sampling uncertainty but do not account for temporal instability of the underlying data.
