...
The monotonic increase from 365d to 30d in traditional OSS---0.4\% $\to$ 1.3\%, a 3$\times$ increase---indicates that context-file activity is \textit{accelerating}, not merely present. Per-repo trends reinforce this: TypeScript surged from 1.9\% (365d) to 11.1\% (30d), React from 0.7\% to 3.2\%, Prometheus from 0.3\% to 2.7\%, Svelte from 0.1\% to 1.3\%, and Django from 0.1\% to 1.3\%. Foundation-governed projects (Linux, Kubernetes, CPython, Go, Spring Boot, Kafka) remain at 0.0\% across all windows. Because the windows overlap, these are not independent samples; however, they provide strong descriptive evidence that the trend direction is upward for developer-led projects.
...
Third, \textit{the governance divide persists}: developer-led projects (mean ACR = 8.4\%, CI: [2.2\%, 18.1\%]) substantially exceed foundation-governed projects (mean ACR = 0.4\%, CI: [0.1\%, 0.6\%]). A Mann-Whitney U test yields $U=27.5$, $z=1.88$, $p=0.060$, rank-biserial $r=0.49$ (medium effect)---marginally significant, with the effect size indicating a meaningful practical difference. Linux (0.0\%), Kubernetes (0.0\%), and Django (0.0\%) show zero detected AI authorship, while Go (0.2\%), Rust (0.2\%), and CPython (0.6\%) show only trace amounts. Notably, Django's agent-era file (\texttt{copilot-instructions.md}) was created explicitly to \textit{discourage} automated AI reviews of pull requests---an instance of governance using the mechanism of context engineering to \textit{limit} rather than enable agent participation. The ACR governance divide is less extreme than CAM's ($p=0.002$) because even foundation-governed repos receive sporadic agent-authored contributions (e.g., Apache Kafka 1.1\%, CPython 0.6\%)---agents submit patches to these projects even without agent-era configuration files, a pattern consistent with externally-driven agent use by individual contributors rather than project-level adoption.
...
\textbf{Internal validity.} The reference repository sample was selected purposively, not randomly. We chose well-known projects across 7 ecosystems to maximize diversity, but the selection is vulnerable to post-hoc bias: we may have unconsciously included projects known to have adopted agent-era files. The governance classification (developer-led vs.\ foundation-governed) was assigned by the authors based on project governance models, not by an independent coder; inter-rater reliability was not assessed. The temporal CAM analysis uses overlapping windows (30d $\subset$ 60d $\subset$ 90d $\subset$ 180d $\subset$ 365d), so the observed ``acceleration'' could reflect a small number of recent file-creation events amplified in shorter windows rather than a sustained trend. The Mann-Whitney U test for the ACR governance comparison is marginally significant ($p=0.060$), and with $n=21$ the test has limited power to detect moderate effects.
