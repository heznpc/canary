...
## Decision: Cowork is out of scope (architectural); Desktop CC links resolve

Initial draft assumed `~/Library/Application Support/Claude/` would yield additional transcript data to widen the dataset beyond CLI. Schema discovery on 2026-05-07 first produced an exclusion in both directions; a same-day reverification corrected one of those calls.

- **`claude-code-sessions/` — link verified, parser feasible.** The first investigation sampled exactly one `cliSessionId` and reported no match in `~/.claude/projects/`. Reverifying across all 45 entries on the same machine showed **45 / 45 resolve** to existing CLI jsonl files. The first finding was a false negative produced by sampling n=1 from a non-representative file. The link is reliable; what `claude-code-sessions` adds is Desktop-only metadata (`originCwd`, `completedTurns`, `permissionMode`, `effort`) on top of the conversation already covered by the CLI parser. Implementing a small enrichment pass is feasible whenever the metadata becomes useful (e.g., correlating leakage with permission mode). This is also a small methodology lesson recorded in-line: do not generalize from n=1 samples in schema discovery.
- **`local-agent-mode-sessions/` (Cowork) — exclusion stands, architectural.** Cowork runs inside the `claudevm.bundle/` VM. Session `cwd` values are VM-internal (`/sessions/<vm-name>`), and any git activity inside Cowork operates on a separate filesystem inside the VM. Cowork sessions therefore cannot, by construction, contribute to host-side push leakage measurement. They are excluded on architectural grounds, not on parser-complexity grounds. A future iteration that wants to measure leakage *within* the Cowork VM (a different research question) would mount the VM image and operate inside it.
